...
@Named
@ViewScoped
public class RelatorioAtividadeExternaBean implements Serializable {
	...
	public RelatorioAtividadeExternaBean() {
	...
	public void emitir() {
		...
		parametros.put("localizacaoSubrelatorioAtividadeExterna", "/relatorios/");
		parametros.put("localizacaoimg1", "/relatorios/");

		
		if(this.todosAtividades.size() > 0) {
			
			GeraRelatorio executor = new GeraRelatorio("/relatorios/relatorio_atividades_externas.jasper",
					"relatorio_atividades_externas",parametros,this.todosAtividades);
	...
	private void filtrar() {
		this.todosAtividades = atividades.filtradosRelatorio(filtro,2);
	}
